...
2. `docs/openclash-stable-settings.md` — 当前实机稳定运行参数、历史不稳定组合与恢复验收基线
3. `README.md` — 使用方法与总体架构
4. `overwrite/openclash-overwrite.conf` — 实际主覆写
5. `rules/manual-direct.yaml` — 当前额外直连规则
6. `docs/acceptance-log.md` — 验收要求
7. `CHANGELOG.md` — 历史变更
...
- 涉及 OpenClash 运行模式、TUN、DNS、Sniffer、Fake-IP、QUIC、IPv6 等运行参数时，先以 `docs/openclash-stable-settings.md` 的实机稳定基线为准，并重新完成其中的最低验收。
...
- 不提交订阅 URL、节点定义、Dashboard Secret、Token、原始 `/etc/openclash/config/*.yaml`、`proxy_provider` 或 OpenWrt/CatWrt 整机备份。
- 修改后按 `docs/CODEX-HANDOFF.md` 和 `docs/openclash-stable-settings.md` 的验收清单验证。
